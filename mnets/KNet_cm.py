...
class KalmanNetNN(torch.nn.Module):
    ...
    def InitKGainNet(self, prior_Q, prior_Sigma, prior_S, args, frozen_weights=None):
        ...
        if self.use_context_mod == True:
            ...
            self.cm_shape = {
                ...
                'lstm_noiseCM_q_w_hh': [d_hidden_Q * 4, d_hidden_Q],
                'lstm_noiseCM_q_b_hh': [d_hidden_Q * 4],
                'lstm_noiseCM_sigma_w_ih': [d_hidden_Sigma * 4, d_input_noiseCM_sigma],
                'lstm_noiseCM_sigma_b_ih': [d_hidden_Sigma * 4],
                'lstm_noiseCM_sigma_w_hh': [d_hidden_Sigma * 4, d_hidden_Sigma],
                'lstm_noiseCM_sigma_b_hh': [d_hidden_Sigma * 4],
        ...
        if self.knet_trainable == False:
            ...
            # Context Modulation
            self.register_parameter('lstm_noiseCM_q_w_ih', nn.Parameter(torch.Tensor(d_hidden_Q * 4, d_input_noiseCM_q)))
            self._weights.append(self.lstm_noiseCM_q_w_ih)
            self.register_parameter('lstm_noiseCM_q_b_ih', nn.Parameter(torch.Tensor(d_hidden_Q * 4)))
            self._weights.append(self.lstm_noiseCM_q_b_ih)
            self.register_parameter('lstm_noiseCM_q_w_hh', nn.Parameter(torch.Tensor(d_hidden_Q * 4, d_hidden_Q)))
            self._weights.append(self.lstm_noiseCM_q_w_hh)
            self.register_parameter('lstm_noiseCM_q_b_hh', nn.Parameter(torch.Tensor(d_hidden_Q * 4)))
            self._weights.append(self.lstm_noiseCM_q_b_hh)
            self.register_parameter('lstm_noiseCM_sigma_w_ih', nn.Parameter(torch.Tensor(d_hidden_Sigma * 4, d_input_noiseCM_sigma)))
            self._weights.append(self.lstm_noiseCM_sigma_w_ih)
            self.register_parameter('lstm_noiseCM_sigma_b_ih', nn.Parameter(torch.Tensor(d_hidden_Sigma * 4)))
            self._weights.append(self.lstm_noiseCM_sigma_b_ih)
            self.register_parameter('lstm_noiseCM_sigma_w_hh', nn.Parameter(torch.Tensor(d_hidden_Sigma * 4, d_hidden_Sigma)))
            self._weights.append(self.lstm_noiseCM_sigma_w_hh)
            self.register_parameter('lstm_noiseCM_sigma_b_hh', nn.Parameter(torch.Tensor(d_hidden_Sigma * 4)))
            self._weights.append(self.lstm_noiseCM_sigma_b_hh)
            self.register_parameter('lstm_noiseCM_s_w_ih', nn.Parameter(torch.Tensor(d_hidden_S * 4, d_input_noiseCM_s)))
            self._weights.append(self.lstm_noiseCM_s_w_ih)
            self.register_parameter('lstm_noiseCM_s_b_ih', nn.Parameter(torch.Tensor(d_hidden_S * 4)))
            self._weights.append(self.lstm_noiseCM_s_b_ih)
            self.register_parameter('lstm_noiseCM_s_w_hh', nn.Parameter(torch.Tensor(d_hidden_S * 4, d_hidden_S)))
    ...
    def step_KGain_est(self, y, SoW):
        ...
        KG = self.KGain_step(obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff, SoW)
    ...
    ### Kalman Net Step ###
    #######################
    def KNet_step(self, y, SoW=None):

        # Compute Priors
        self.step_prior()

        # Compute Kalman Gain
        self.step_KGain_est(y, SoW)
    ...
    ### Kalman Gain Step ###
    ########################
    def KGain_step(self, obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff, SoW):
        ...
        if self.use_context_mod == True:
            ...
            # CM Q-lstm
            in_noiseCM_q = SoW
            self.out_noiseCM_Q, self.h_noiseCM_Q = self.lstm_rnn_step(in_noiseCM_q, (self.out_noiseCM_Q, self.h_noiseCM_Q),
            [self.lstm_noiseCM_q_w_ih,
                self.lstm_noiseCM_q_b_ih,
                self.lstm_noiseCM_q_w_hh,
                self.lstm_noiseCM_q_b_hh])
            
            # Update hidden state of Q-lstm
            self.h_Q = self.h_Q + self.out_noiseCM_Q
            ...
            # CM S-lstm
            in_noiseCM_s = SoW
            self.out_noiseCM_S, self.h_noiseCM_S = self.lstm_rnn_step(in_noiseCM_s, (self.out_noiseCM_S, self.h_noiseCM_S),
            [self.lstm_noiseCM_s_w_ih,
                self.lstm_noiseCM_s_b_ih,
                self.lstm_noiseCM_s_w_hh,
                self.lstm_noiseCM_s_b_hh])
            
            # Update hidden state of S-lstm
            self.h_S = self.h_S + self.out_noiseCM_S
            ...
            # CM Sigma-lstm
            in_noiseCM_sigma = SoW
            self.out_noiseCM_Sigma, self.h_noiseCM_Sigma = self.lstm_rnn_step(in_noiseCM_sigma, (self.out_noiseCM_Sigma, self.h_noiseCM_Sigma),
            [self.lstm_noiseCM_sigma_w_ih,
                self.lstm_noiseCM_sigma_b_ih,
                self.lstm_noiseCM_sigma_w_hh,
                self.lstm_noiseCM_sigma_b_hh])
            self.h_Sigma = out_FC4 + self.out_noiseCM_Sigma
    ...
    ### Forward ###
    ###############
    def forward(self, y, SoW=None):
        y = y.to(self.device)

        # case: Knet weights are frozen, train only the CM weights
        if SoW is not None:
            assert(self.knet_trainable == False)
            SoW = torch.log10(SoW).unsqueeze(0)
            return self.KNet_step(y, SoW)
