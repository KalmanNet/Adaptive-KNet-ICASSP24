...
class KalmanNetNN(torch.nn.Module):
    ...
    def split_cm_weights(self, weights):   
        self.fc1_gain = weights[0].to(self.device)
        self.fc1_shift = weights[1].to(self.device)
        self.fc2_gain1 = weights[2].to(self.device)
        self.fc2_shift1 = weights[3].to(self.device)
        self.fc3_gain = weights[4].to(self.device)
        self.fc3_shift = weights[5].to(self.device)
        self.fc4_gain = weights[6].to(self.device)
        self.fc4_shift = weights[7].to(self.device)
        self.fc5_gain = weights[8].to(self.device)
        self.fc5_shift = weights[9].to(self.device)
        self.fc6_gain = weights[10].to(self.device)
        self.fc6_shift = weights[11].to(self.device)
        self.fc7_gain = weights[12].to(self.device)
        self.fc7_shift = weights[13].to(self.device)
        self.lstm_q_ih_gain = weights[14].to(self.device)
        self.lstm_q_ih_shift = weights[15].to(self.device)
        self.lstm_q_hh_gain = weights[16].to(self.device)
        self.lstm_q_hh_shift = weights[17].to(self.device)
        self.lstm_sigma_ih_gain = weights[18].to(self.device)
        self.lstm_sigma_ih_shift = weights[19].to(self.device)
        self.lstm_sigma_hh_gain = weights[20].to(self.device)
        self.lstm_sigma_hh_shift = weights[21].to(self.device)
        self.lstm_s_ih_gain = weights[22].to(self.device)
        self.lstm_s_ih_shift = weights[23].to(self.device)
        self.lstm_s_hh_gain = weights[24].to(self.device)
        self.lstm_s_hh_shift = weights[25].to(self.device)
