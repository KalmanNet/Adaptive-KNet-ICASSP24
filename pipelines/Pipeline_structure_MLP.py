"""
This file contains the class Pipeline_structure_MLP, 
which is used to train and test the hyper-KalmanNet model with CM layers generated by structured MLPs.
...
class Pipeline_structure_MLP:
    ...
    def NNTrain_mixdatasets(self, SoW_train_range, sys_model, cv_input_tuple, cv_target_tuple, train_input_tuple, train_target_tuple, path_results, \
        ...
        for ti in range(0, self.N_steps):
            ...
            for i in SoW_train_range: # dataset i  
                ...
                # Forward Computation
                weights_cm = self.hnet(train_input_tuple[i][1]) # input SoW, output CM weights
            ...
            with torch.no_grad():
                for i in SoW_train_range: # dataset i 
                    ...
                    weights_cm = self.hnet(cv_input_tuple[i][1]) # input SoW, output CM weights
    ...
    def NNTrain_curricumlum(self, SoW_train_range, sys_model, cv_input_tuple, cv_target_tuple, train_input_tuple, train_target_tuple, path_results, \
        cv_init, train_init, MaskOnState=False, train_lengthMask=None,cv_lengthMask=None):
        # Curriculum Learning for HNet
        ...
        for i in SoW_train_range: # dataset i 
            ...
            # data size
            self.N_E = len(train_input_tuple[i][0]) # Number of Training Sequences
            self.N_CV = len(cv_input_tuple[i][0]) # Number of Validation Sequences
            sysmdl_T_test = cv_input_tuple[i][0].shape[2]  
            ...
            for ti in range(0, self.N_steps):               
                # each turn, train on dataset i for N steps
                #################
                ### Training  ###
                #################  
                self.optimizer.zero_grad()         
    ...
    def NNTest_alldatasets(self, SoW_test_range, sys_model, test_input_tuple, test_target_tuple, path_results,test_init,\
        ...
        for i in SoW_test_range: # dataset i   
            ...
            weights_cm = self.hnet(SoW_test) # input SoW, output CM weights
